...
public abstract class ClinicScheduleInflaterAsserter {
    ...
    private static void assertCategory(ViewGroup category, String categoryName) {
        assertNotNull(category);
        TextView title = category.findViewById(R.id.agenda_letter_fragment_agenda_textview);
    ...
    private static void assertCards(ViewGroup category, List<Clinica> clinicsExpectsInOrder) {
        ViewGroup layout = category.findViewById(R.id.agenda_letter_fragment_linear_layout);
        ...
        for(int i = 0; i < layout.getChildCount(); i++) {
            ...
            assertEquals(R.id.card_fragment_clinic, card.getChildAt(0).getId());
            TextView textViewName = card.findViewById(R.id.card_fragment_clinic_name);
            assertEquals(clinicExpected.getNome(), textViewName.getText().toString());
            TextView textViewCity = card.findViewById(R.id.card_fragment_clinic_city);
            assertEquals(clinicExpected.getCidade(), textViewCity.getText().toString());
            TextView textViewNeighborHood = card.findViewById(R.id.card_fragment_clinic_neighborhood);
